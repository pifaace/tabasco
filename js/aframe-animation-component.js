			...
			AFRAME.registerComponent("animation", {
				...
				update: function () {
					...
					var c = {
						autoplay: !1,
						begin: function () {
							e.emit("animation-begin"),
							e.emit(t + "-begin")
						},
						complete: function () {
							e.emit("animation-complete"),
							e.emit(t + "-complete"),
				...
				play: function () {
					this.animation && this.animationIsPlaying && (this.playAnimation(), this.addEventListeners())
				},
				...
				playAnimation: function () {
					this.animation && (this.animation.restart(), this.animationIsPlaying = !0)
				},
				pauseAnimation: function () {
					this.animation && (this.animation.pause(), this.animationIsPlaying = !1)
				}
			})
		}, function (t, n, e) {
			var r,
			...
			(this, function () {
				...
				i = function () {
					return {
						...
						color: function (t) {
							return i.hex(t) || i.rgb(t) || i.rgba(t) || i.hsl(t)
						}
					}
				}
				(),
				o = function () {
					...
					e = {
						...
						Elastic: function (t, n) {
							...
							return  - (Math.pow(2, 10 * a) * Math.sin((a - i) * (2 * Math.PI) / e))
						},
						Back: function (t) {
							return t * t * (3 * t - 2)
						},
					...
					Object.keys(e).forEach(function (n) {
						...
						t["easeInOut" + n] = function (t, n) {
							return t < .5 ? r(2 * t, n) / 2 : 1 - r(t * -2 + 2, n) / 2
						}
					}),
					t.linear = function (t) {
						return t
				...
				l = function (t) {
					return t.reduce(function (t, n) {
						return t.concat(i.array(n) ? l(n) : n)
					}, [])
				},
				p = function (t) {
					return i.array(t) ? t : (i.string(t) && (t = c(t) || t), t instanceof NodeList || t instanceof HTMLCollection ? [].slice.call(t) : [t])
				},
				m = function (t, n) {
				...
				A = function (t) {
					return i.rgb(t) || i.rgba(t) ? t : i.hex(t) ? y(t) : i.hsl(t) ? b(t) : void 0
				},
				E = function (t) {
					return /([\+\-]?[0-9|auto\.]+)(%|px|pt|em|rem|in|cm|mm|ex|pc|vw|vh|deg)?/.exec(t)[2]
				},
				M = function (t, n, e) {
					return E(n) ? n : t.indexOf("translate") > -1 ? E(e) ? n + E(e) : n + "px" : t.indexOf("rotate") > -1 || t.indexOf("skew") > -1 ? n + "deg" : n
				},
				x = function (t, n) {
					if (n in t.style)
				...
				w = function (t, n) {
					...
					if (!r)
						return e;
					for (var a = /(\w+)\((.+?)\)/g, i = [], o = [], u = []; i = a.exec(r); )
				...
				I = function (t, n) {
					return i.dom(t) && m(r, n) ? "transform" : i.dom(t) && "transform" !== n && x(t, n) ? "css" : i.dom(t) && (t.getAttribute(n) || i.svg(t) && t[n]) ? "attribute" : i["null"](t[n]) || i.undef(t[n]) ? void 0 : "object"
				},
				P = function (t, n) {
					switch (I(t, n)) {
					case "transform":
				...
				F = function (t) {
					var t = t ? l(i.array(t) ? t.map(p) : p(t)) : [];
				...
				C = function (t, n, e, r) {
					var a = p(i.func(e) ? e(t, r) : e);
					return {
						from: a.length > 1 ? a[0] : P(t, n),
				...
				R = function (t, n, e, r) {
					var a = {};
					if ("transform" === e)
						a.from = t + "(" + M(t, n.from, n.to) + ")", a.to = t + "(" + M(t, n.to) + ")";
					else {
				...
				B = function (t, n) {
					...
					return t.forEach(function (r, a) {
						...
						return n.forEach(function (n) {
							var u = I(o, n.name);
							if (u) {
								var s = C(o, n.name, n.value, a),
								c = v(n);
								c.animatables = r,
								c.type = u,
								c.from = R(n.name, s, c.type, o).from,
								c.to = R(n.name, s, c.type, o).to,
								c.round = i.color(s.from) || c.round ? 1 : 0,
								c.delay = (i.func(c.delay) ? c.delay(o, a, t.length) : c.delay) / J.speed,
								c.duration = (i.func(c.duration) ? c.duration(o, a, t.length) : c.duration) / J.speed,
								e.push(c)
							}
						})
					}),
				...
				$ = function (t) {
					...
					return t.tweens.forEach(function (t) {
						"css" !== t.type && "transform" !== t.type || (n.push("css" === t.type ? s(t.name) : "transform"), t.animatables.forEach(function (t) {
								e.push(t.target)
							}))
					}), {
						properties: h(n).join(", "),
				...
				q = function (n, e) {
					...
					if (r) {
						...
						for (var i in r)
							n.animatables[i].target.style[t] = r[i].join(" ")
					}
					n.settings.update && n.settings.update(n)
				},
				D = function (t) {
					var n = {};
					return n.animatables = F(t.targets),
					n.settings = g(t, e),
					n.properties = j(t, n.settings),
					n.tweens = N(n.animatables, n.properties),
					n.duration = T(n.tweens) || t.duration,
					n.currentTime = 0,
					n.progress = 0,
					n.ended = !1,
					n
				...
				J = function (t) {
					...
					return n.tick = function (t) {
						...
						e.current >= n.duration && (r.loop ? (e.start = t, "alternate" === r.direction && S(n, !0), i.number(r.loop) && r.loop--) : (n.ended = !0, n.pause(), r.complete && r.complete(n)), e.last = 0)
					},
					n.seek = function (t) {
						q(n, t / 100 * n.duration)
					},
					n.pause = function () {
						X(n);
						var t = z.indexOf(n);
					...
					n.play = function (t) {
						n.pause(),
						t && (n = g(D(g(t, n.settings)), n)),
						e.start = 0,
						e.last = n.ended ? 0 : n.currentTime;
						var r = n.settings;
						"reverse" === r.direction && S(n),
				...
				K = function (t) {
					for (var n = l(i.array(t) ? t.map(p) : p(t)), e = z.length - 1; e >= 0; e--)
						for (var r = z[e], a = r.tweens, o = a.length - 1; o >= 0; o--)
							for (var u = a[o].animatables, s = u.length - 1; s >= 0; s--)
								m(n, u[s].target) && (u.splice(s, 1), u.length || a.splice(o, 1), a.length || r.pause())
				};
				return J.version = n,
				J.speed = 1,
				J.list = z,
